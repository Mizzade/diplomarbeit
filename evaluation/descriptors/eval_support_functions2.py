...
def get_collection_names(config:Dict, sorted_output:bool=True) -> List[str]:
    ...
    Arguments:
        config {Dict} -- See scripts/eval/config_eval_detescriptors.py
...
def build_evaluater_for_descriptors(config:Dict) -> Evaluater:
    fs = build_file_system(config)
    return Evaluater('descriptors', config, fs)
