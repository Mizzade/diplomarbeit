...
addpath(genpath('./vlfeat-0.9.21/toolbox/mex/'));

dir_name = ['../data/'];
dir_images =['./images/']; % folder containing images
dir_output = ['./output_det/']; % where to save this step's output.
dir_data =['./output/']; % Folder where the .mat files from first step of TCovDet lie.
...
elseif strcmp(datasets_name, 'webcam')
...
% image_list = load_image_list([dir_images datasets_name '/'], datasets_name);
[s, mess, messid] = mkdir([dir_output save_feature_name '/' datasets_name '/']);

for set_index = 1:numel(subsets)
    subset = subsets{set_index};
    image_list = load_image_list([dir_images datasets_name '/'], [subset '/']);
    [s, mess, messid] = mkdir([dir_output save_feature_name '/' datasets_name '/' subset '/']);

    for i = 1:numel(image_list)
        feature = [];
        score = [];
        try
            % disp([dir_images datasets_name '/' subset '/' image_list(i).name]);
            image = imread([dir_images  datasets_name '/' subset '/' image_list(i).name]);
            x = load([dir_data load_feature_name '/' datasets_name '/' subset '/' image_list(i).name(1:end-4) '.mat']);
        catch
            % disp(image_list(i).name);
            disp(['Bad load: ' dir_data load_feature_name '/' datasets_name '/' subset '/' image_list(i).name(1:end-4) '.mat']);
            % disp('bad load');
            save([dir_output save_feature_name '/' datasets_name '/' subset '/' image_list(i).name(1:end-4) '.mat'],'feature','score');
            continue;
        end
        ...
        disp(['output: ', output])
        if numel(output)==0
            disp(['Output number is 0 for ' image_list(i).name]);
            save([dir_output save_feature_name '/' datasets_name '/' subset '/' image_list(i).name(1:end-4) '.mat'],'feature','score');
            continue;
        end
